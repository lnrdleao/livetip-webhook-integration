...
async function testBitcoin400() {
    try {
        console.log('🚀 Testando pagamento Bitcoin - 400 sats');
        
        // Gerar ID único para este teste
        const uniqueId = `BTC_400_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
        console.log(`🔑 ID Único: ${uniqueId}`);
          const payload = {
            userName: "Teste 400 Sats",
        ...
        if (response.data.lightning_invoice) {
            ...
            console.log(`💰 Valor: 400 satoshis`);
            console.log(`🔑 ID Único: ${uniqueId}`);

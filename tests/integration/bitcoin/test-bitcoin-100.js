...
async function testBitcoin100() {
    try {
        console.log('🚀 Testando pagamento Bitcoin - 100 sats');
        
        // Gerar ID único para este teste
        const uniqueId = `BTC_100_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
        console.log(`🔑 ID Único: ${uniqueId}`);
          const payload = {
            userName: "Teste 100 Sats",
        ...
        if (response.data.lightning_invoice) {
            ...
            console.log(`💰 Valor: 100 satoshis`);
            console.log(`🔑 ID Único: ${uniqueId}`);

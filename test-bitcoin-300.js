...
async function testBitcoin300() {
    try {
        console.log('🚀 Testando pagamento Bitcoin - 300 sats');
        
        // Gerar ID único para este teste
        const uniqueId = `BTC_300_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
        console.log(`🔑 ID Único: ${uniqueId}`);
          const payload = {
            userName: "Teste 300 Sats",
        ...
        if (response.data.lightning_invoice) {
            ...
            console.log(`💰 Valor: 300 satoshis`);
            console.log(`🔑 ID Único: ${uniqueId}`);
